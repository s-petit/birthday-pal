...
`CardDAV` protocol and `vCard` files are the standard for many contact providers.
...
- Host URLs, ports, and credentials (only `BasicAuth` is supported)
...
- `CardDAV/VCard` in order to read contacts
- `BasicAuth` for authentication
- `SMTP` for email reminders

You just have to provide :

- A `CardDAV` server URL with credentials (BasicAuth)
...
- Contacts (`CardDAV`) server HTTP URL, with credentials (`BasicAuth`)
...
* * *
Automatically generated by [autoreadme](https://github.com/jimmyfrasche/autoreadme) on 2021.07.10
